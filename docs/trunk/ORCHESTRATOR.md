...
| `ve orch work-unit delete <chunk>` | Remove a work unit (safe by default) |
...
By default, `ve orch work-unit delete` refuses to delete a work unit if its branch has unmerged commits. This prevents accidental loss of implementation work.

```bash
# Safe delete (default) - refuses if branch has unmerged commits
ve orch work-unit delete my_chunk

# If the branch has unmerged commits, you'll see an error like:
# Error: Branch has 3 unmerged commit(s). Use force=true to delete anyway, or merge changes first.

# Force delete - use when you intentionally want to discard unmerged work
ve orch work-unit delete my_chunk --force
```

**When to use `--force`:**
- The work is intentionally being abandoned
- The changes have been cherry-picked or rebased elsewhere
- The branch represents failed/incorrect implementation

**Alternatives to force delete:**
- Use `ve orch prune` to merge retained worktrees back to base
- Manually merge or cherry-pick valuable commits before deleting
